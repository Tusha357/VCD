...
# Function to activate Jarvis (wake up)
def activate_jarvis():
    global jarvis_active
    jarvis_active = True
    speak("Jarvis is now awake and ready for your commands.")
    print("Jarvis is awake")

# Function to deactivate Jarvis (sleep)
def deactivate_jarvis():
    global jarvis_active
    jarvis_active = False
    speak("Jarvis is now asleep.")
...
# Function to listen for both wake word and commands
def listen_for_wake_word_and_commands():
    global jarvis_active
    while True:
        try:
            with mic as source:
                recognizer.adjust_for_ambient_noise(source)
                print("Listening for wake word or commands...")
                ...
                # If Jarvis is inactive, listen for wake word
                if not jarvis_active:
                    if "wake up jarvis" in command or "jarvis" in command:
                        activate_jarvis()
                # If Jarvis is active, listen for sleep command or other commands
                else:
                    if "sleep jarvis" in command:
                        deactivate_jarvis()
                    else:
...
if __name__ == "__main__":
    listen_for_wake_word_and_commands()
